/**
 * Core diff algorithm interfaces and classes.
 * Ported from VS Code's diff algorithm.
 */
import { OffsetRange } from './offsetRange';

/**
 * Represents a sequence that can be diffed.
 */
...
/**
 * Represents a timeout for diff computation.
 */
...
/**
 * A timeout based on elapsed time.
 */
...
/**
 * Represents a pair of offsets in two sequences.
 */
...
export class SequenceDiff {
  public static invert(sequenceDiffs: SequenceDiff[], doc1Length: number): SequenceDiff[] {
    ...
    let lastEndExclusive1 = 0;
    let lastEndExclusive2 = 0;

    for (const diff of sequenceDiffs) {
      result.push(
        SequenceDiff.fromOffsetPairs(
          new OffsetPair(lastEndExclusive1, lastEndExclusive2),
          diff.getStarts()
        )
      );
      lastEndExclusive1 = diff.seq1Range.endExclusive;
      lastEndExclusive2 = diff.seq2Range.endExclusive;
    }

    result.push(
      SequenceDiff.fromOffsetPairs(
        new OffsetPair(lastEndExclusive1, lastEndExclusive2),
        new OffsetPair(doc1Length, lastEndExclusive2 + (doc1Length - lastEndExclusive1))
      )
    );
